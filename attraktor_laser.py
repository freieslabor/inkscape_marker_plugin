...
Based on a script by Nick Drobchenko from the CNC club and Peter Rogers at
thinkhaus.org
...
* removed "area curve" code, since it wasn"t being used and wasn"t working
  anyways
...
* removed extra M3s: the laser only turns on at the beginning of a cut block,
  not before every line.

Changelog 2010-04-07 - Adina:
* separate scaling factor for x and y,
* x and y scaled to mm or inches that match the dimensions in inkscape
  (not 1px = 1 in or mm)
...
class P:
    ...
    Pretty much what it sounds like: defines some arithmetic functions that can
...
def cspbezsplit(sp1, sp2, t=0.5):
    ...
    return [[sp1[0][:], sp1[1][:], list(s1[1])], [list(s1[2]), list(s1[3]),
            list(s2[1])], [list(s2[2]), sp2[1][:], sp2[2][:]]]
...
def distance_from_point_to_arc(p, arc):
    ...
    if r > 0:
        ...
        if between(alpha, 0, a) or \
                min(abs(alpha), abs(alpha-a)) < STRAIGHT_TOLERANCE:
...
def get_distance_from_csp_to_arc(sp1, sp2, arc1, arc2, tolerance=0.001):
    ...
    while i < 1 or (abs(d1[0] - dl[0]) > tolerance and i < 2):
        ...
        for j in range(n+1):
            ...
            d = min(distance_from_point_to_arc(p, arc1),
                    distance_from_point_to_arc(p, arc2))
            d1 = max(d1, d)
        n = n*2
    return d1[0]
...
def biarc(sp1, sp2, z1, z2, depth=0,):
    ...
    def biarc_split(sp1, sp2, z1, z2, depth):
        if depth < options.biarc_max_split_depth:
            ...
            return biarc(sp1, sp2, depth + 1, z1, zm) \
                + biarc(sp2, sp3, depth + 1, z1, zm)
        else:
    ...
    TS = TS.unit()
    TE = TE.unit()
    tang_are_parallel = ((tsa - tea) % math.pi < STRAIGHT_TOLERANCE or
    ...
    if (tang_are_parallel and
            ((v.mag() < STRAIGHT_DISTANCE_TOLERANCE or
             ...
             1 - abs(TS * v / (TS.mag() * v.mag())) < STRAIGHT_TOLERANCE)):
                ...
                # Both tangents are parallel and start and end are the same -
                # line straight or one of tangents still smaller then tolerance
    ...
    d = get_distance_from_csp_to_arc(sp1, sp2, [P0, P2, R1, a1],
                                     [P2, P4, R2, a2])
...
class Gcode_tools(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("-d", "--directory", action="store",
                                     type="string", dest="directory",
                                     default=outdir, help="Directory for gcode file")  # noqa
        self.OptionParser.add_option("-f", "--filename", action="store",
                                     type="string", dest="file",
                                     default="-1.0", help="File name")
        self.OptionParser.add_option("-u", "--Xscale", action="store",
                                     type="float", dest="Xscale",
                                     default="1.0", help="Scale factor X")
        self.OptionParser.add_option("-v", "--Yscale", action="store",
                                     type="float", dest="Yscale",
                                     default="1.0", help="Scale factor Y")
        self.OptionParser.add_option("-x", "--Xoffset", action="store",
                                     type="float", dest="Xoffset",
                                     default="0.0", help="Offset along X")
        self.OptionParser.add_option("-y", "--Yoffset", action="store",
                                     type="float", dest="Yoffset",
                                     default="0.0", help="Offset along Y")
        self.OptionParser.add_option("-p", "--feed", action="store",
                                     type="float", dest="feed", default="4.0",
                                     help="Feed rate in unit/min")

        self.OptionParser.add_option("", "--biarc-tolerance", action="store",
                                     type="float", dest="biarc_tolerance",
                                     default="1",
                                     help="Tolerance used when calculating biarc interpolation.")  # noqa
        self.OptionParser.add_option("", "--biarc-max-split-depth",
                                     action="store", type="int",
                                     dest="biarc_max_split_depth",
                                     default="4",
                                     help="Defines maximum depth of splitting while approximating using biarcs.")  # noqa

        self.OptionParser.add_option("", "--unit", action="store",
                                     type="string", dest="unit",
                                     default="G21 (All units in mm)\n",
                                     help="Units")
        self.OptionParser.add_option("", "--function", action="store",
                                     type="string", dest="function",
                                     default="Curve",
                                     help="What to do: Curve|Area|Area inkscape")  # noqa
        self.OptionParser.add_option("", "--tab", action="store",
                                     type="string", dest="tab", default="",
                                     help="Means nothing right now. Notebooks Tab.")  # noqa
        self.OptionParser.add_option("", "--generate_not_parametric_code",
                                     action="store", type="inkbool",
                                     dest="generate_not_parametric_code",
                                     default=False,
                                     help="Generated code will be not parametric.")  # noqa
        self.OptionParser.add_option("", "--double_sided_cutting",
                                     action="store", type="inkbool",
                                     dest="double_sided_cutting",
                                     default=False,
                                     help="Generate code for double-sided cutting.")  # noqa
        self.OptionParser.add_option("", "--draw-curves", action="store",
                                     type="inkbool", dest="drawCurves",
                                     default=False,
                                     help="Draws curves to show what geometry was processed")  # noqa
        self.OptionParser.add_option("", "--logging", action="store",
                                     type="inkbool", dest="logging",
                                     default=False,
                                     help="Enable output logging from the plugin")  # noqa

        self.OptionParser.add_option("", "--loft-distances", action="store",
                                     type="string", dest="loft_distances",
                                     default="10",
                                     help="Distances between paths.")
        self.OptionParser.add_option("", "--loft-direction", action="store",
                                     type="string", dest="loft_direction",
                                     default="crosswise",
        ...
        self.OptionParser.add_option("", "--loft-interpolation-degree",
                                     ...
                                     default="2",
                                     help="Which interpolation use to loft the paths smooth interpolation or staright.")  # noqa

        self.OptionParser.add_option("", "--min-arc-radius", action="store",
                                     type="float", dest="min_arc_radius",
                                     default=".1",
                                     help="All arc having radius less than minimum will be considered as straight line")  # noqa

    def parse_curve(self, path):
        # if self.options.Xscale != self.options.Yscale:
        #     xs, ys = self.options.Xscale,self.options.Yscale
        #     self.options.Xscale = self.options.Yscale = 1.0
        # else :
        xs = ys = 1.0
        ...
        #               start = p[i][0][1]
        #
        #               dist = max((-((end[0] - start[0])**2 +
        #                          (end[1] - start[1])**2), i), dist)
        #           np += [p[dist[1]][:]]
        #           del p[dist[1]]
        #     p = np[:]
        ...
        for subpath in path:
            ...
            for i in range(1, len(subpath)):
                sp1 = [[subpath[i-1][j][0] * xs, subpath[i-1][j][1] * ys]
                       for j in range(3)]
                sp2 = [[subpath[i][j][0] * xs, subpath[i][j][1] * ys]
                       for j in range(3)]
        ...
        for si in curve:
            if s != "":
                if s[1] == "line":
                    element_properties = {
                        "style": style["line"],
                        "d": "M %s,%s L %s,%s" % (s[0][0], s[0][1], si[0][0],
                                                  si[0][1]),
                        "comment": str(s)
                    }
                    ...
                    a_st = (math.atan2(sp[0] - c[0],
                            -(sp[1] - c[1])) - math.pi / 2) % (math.pi*2)
    ...
    def make_args(self, c):
        """
        Turns a list of arguments into gcode-style parameters
        (eg (1, 2, 3) -> "X1 Y2 Z3"), taking scaling, offsets and the
        "parametric curve" setting into account
        """
        c = [c[i] if i < len(c) else None for i in range(6)]
        if c[5] == 0:
            c[5] = None

        # next few lines generate the stuff at the front of the file - scaling,
        # offsets, etc (adina)
        ...
        # I think this is the end of generating the header stuff
        # (adina, june 22 2010)
        args = []
        for i in range(6):
            if c[i] is not None:
    ...
    def generate_gcode(self, curve, depth, altfeed=None):
        ...
        ccwArc = "G03"
        if (self.flipArcs):
            # The geometry is reflected, so invert the orientation of the arcs
            # to match
            (cwArc, ccwArc) = (ccwArc, cwArc)

        # Peter's note: Here's where the "laser on" and "laser off" m-codes get
        ...
        for i in range(1, len(curve)):
            ...
            if s[1] == "move":
                ...
                # up, going down, or indexing to a new workplace.  All other
                # cases seem to signal cutting.
                ...
                dx = s[2][0] - s[0][0]
                dy = s[2][1] - s[0][1]
                if abs((dx**2 + dy**2) * self.options.Xscale) > \
                        self.options.min_arc_radius:
                    r1 = P(s[0]) - P(s[2])
                    r2 = P(si[0]) - P(s[2])
                    if abs(r1.mag() - r2.mag()) < 0.001:
                        if (s[3] > 0):
                            ...
                            gcode += ccwArc
                        gcode += " " + self.make_args(
                            si[0] + [None, dx, dy, None]) + feed + "\n"
                        ...
                        gcode += " " + self.make_args(si[0]) + " R%f" % \
                            (r * self.options.Xscale) + feed + "\n"

                    lg = cwArc
                else:
                    if lg == "G00":
                        # gcode += "G01 " + self.make_args(
                        #     [None, None, s[5][0] + depth]) + feed + "\n" + \
                        #     LASER_ON
                        gcode += LASER_ON
    ...
    def effect_curve(self, selected):
        ...
        if self.options.drawCurves and len(selected) > 0:
            self.biarcGroup = inkex.etree.SubElement(selected[0].getparent(),
                                                     SVG_GROUP_TAG)
            options.Group = self.biarcGroup

        # Recursively compiles a list of paths that are decendant from the
        # given node
        self.skipped = 0
        ...
        # Compile a list of layers in this document. We compile a list of only
        # the layers we need to use, so we can know ahead of time whether to
        # put tool change operations between them.
        layers = []
        ...
        for layer in layers:
            ...
            # Parse the layer label text, which consists of the layer name
            # followed by an optional number of arguments in square brackets.
            try:
            ...
            # Check if the layer specifies an alternative (from the default)
            # feed rate
            ...
            if (len(layers) > 1):
                ...
                # Move the laser into the starting position (so that way it is
                # positioned for testing the power level, if the user wants to
                # change that).
                arg = curve[0]
                pt = arg[0]
        ...
        if (selected):
            ...
            if (pathList):
                ...
                gcode += "\n(*** Root objects ***)\n"
                if (layers):
                    # Include a tool change operation between the layers
                    # outputted above and these "orphaned" objects.
                    gcode += self.tool_change()
...
#8  = %f (Offset x)
#9  = %f (Offset y)
        \n""" % (self.options.Xscale, self.options.Yscale, self.options.feed,
                 self.options.Xoffset, self.options.Yoffset)
